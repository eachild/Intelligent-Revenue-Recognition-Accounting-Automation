...
def allocate_relative_ssp(ssps:List[float], total:float)->List[float]:
    ...
    for i,v in enumerate(ssps):
        if i<len(ssps)-1:
            a=round(total*(v/s),2); out.append(a); run+=a
...
def straight_line(price:float,start:date,end:date)->Dict[str,float]:
    ...
    k=f"{months[-1].year}-{months[-1].month:02d}"; out[k]=round(price-sum(v for kk,v in out.items() if kk!=k),2); 
    return out
...
def percent_complete(price:float, sched:List[Dict])->Dict[str,float]:
    out={}; prev=0.0
    for r in sched:
        cum=float(r.get("percent_cumulative",0.0)); delta=max(0.0,cum-prev)
        out[r["period"]]=round(price*delta,2); prev=cum
    return out
