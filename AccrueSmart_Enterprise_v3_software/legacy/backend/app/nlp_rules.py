...
def find_currency(text:str):
    m=re.search(r'\b(USD|EUR|GBP|INR|JPY|CAD|AUD)\b', text, re.I)
...
def find_total_price(text:str):
    ...
    nums=[float(a) for a in amounts] if amounts else []
...
def extract_pos(text:str):
    ...
    for po in out:
        ...
        seen.add(po['description'])
        dedup.append(po)
    return dedup
def detect_risks(text:str):
    findings=[]
    import re as _r
    def add(t, s, sev='medium', c=None):
        findings.append({'type':t,'snippet':s[:200],'severity':sev,'comment':c})
...
def extract_commission(text:str):
    import re as _r
    m=_r.search(r'(sales commission|commission)\s*(?:of|:)\s*\$\s?([0-9,]+\.?[0-9]{0,2})', text, _r.I)
    return to_float(m.group(2)) if m else None
def recommendations(text:str):
    recs=[]
    low=text.lower()
    if 'right to return' in low and 'restocking fee' not in low:
        recs.append({'issue':'Right of return','suggested_language':'Add restocking fee and defined return window; specify estimation method.','rationale':'Reduce reversal risk'})
    if 'acceptance' in low and 'criteria' not in low:
        recs.append({'issue':'Acceptance without criteria','suggested_language':'Define objective criteria and testing responsibility; link recognition to formal acceptance.','rationale':'Prevent premature recognition'})
    return recs
