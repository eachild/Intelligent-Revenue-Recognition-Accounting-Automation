...
def save_lock(contract_id: str, schedule: Dict[str, Any], approver_sub: str, approver_email: Optional[str], note: Optional[str]=None) -> Dict[str, Any]:
    ...
    #         "note": lock.note,
    #     }
    print("Mocking save_lock. No database was called.")
    return {
        "id": 1,
        "contract_id": contract_id,
        "hash": digest,
...
def get_lock_status(contract_id: str) -> Dict[str, Any]:
    ...
    #     stmt = select(ScheduleLock).where(ScheduleLock.contract_id == contract_id).order_by(ScheduleLock.locked_at.desc())
    ...
    #         "note": row.note or "",
    #     }
    print(f"Mocking get_lock_status for {contract_id}. No database was called.")
    return {"locked": False}