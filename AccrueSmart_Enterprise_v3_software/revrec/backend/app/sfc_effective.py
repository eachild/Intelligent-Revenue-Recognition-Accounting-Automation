...
def monthly_rate_from_annual(annual: float) -> float: return annual/12.0

def infer_monthly_irr(cashflows: List[float]) -> float:
    try:
        irr = np.irr(cashflows); return float(irr) if irr is not None else 0.0
